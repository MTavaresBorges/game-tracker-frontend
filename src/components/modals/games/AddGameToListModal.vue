<script setup>
    const emit = defineEmits(['close-list-modal']);
            ...
            <div v-show="modalActive" class="fixed w-full bg-black bg-opacity-30 h-screen top-0 left-0 flex items-center justify-center px-8">
                <Transition name="modal-inner">
                    <div v-if="modalActive" class="p-4 bg-gray-800 rounded-xl shadow-2xl text-white self-start self-start mt-[400px] max-w-screen-md">
                        <h2 class="text-lg font-bold">Which library do you want to add this game to?</h2>
                        <p>If you don't remember the date, you can let it blank.</p>
                        <slot />
                        <button class="bg-gray-700 rounded-xl ml-4 hover:bg-gray-600 transition duration-300 text-white mt-8 py-2 px-6" @click="$emit('close-list-modal')">Close</button>
                    </div>
                </Transition>
            </div>
        </Transition>
